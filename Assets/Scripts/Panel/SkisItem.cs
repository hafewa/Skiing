...
public class SkisItem : MonoBehaviour, IPointerClickHandler, IPointerDownHandler
{
    public Image Dot;
    ...
    void BuyBtnClick() {
        ...
        if (skinModel.Price > playerGold) {
            ...
            //TODO弹出购买界面
            rolePanel.OpenBuyPanel(BuyType.Skis, skinModel.Price, skinModel.ID, BuyBtn);
        }
    }
    ...
    public void RefreshUI(int passStageCount, string[] ownedSkinIDs) {
        ...
        SkinIcon.sprite = Resources.Load<Sprite>("UI/Skis/" + skinModel.IconName);
    ...
    public void OnPointerClick(PointerEventData eventData) {
        ...
        if (Lock.gameObject.activeSelf) {
            ...
            MessageTool.ShowMessage(string.Format("通过{0}关解锁新滑雪板：{1}!", skinModel.UnlockStage, skinModel.Name), 1f);
            return;
        }
        ...
        if (ownedSkisIDs.Contains(skinModel.ID.ToString())) {
            Player.Instance.SetPlayerSkis(skinModel.ID);
            rolePanel.ShowSkisProperty(skinModel.ID);
        }
    }
